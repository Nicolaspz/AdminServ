...
export default function ProductsList() {
  ...
  const [productsIgre, setProductsIgre] = useState<Product[]>([]);
  ...
  async function fetchData() {
    try {
      ...
      // Limpa os estados para evitar inconsistências
      setProducts([]);
      setFilteredProducts([]);
      setCategories([]);
    } finally {
  ...
  async function fetchDataIgre() {
    ...
    try {
      ...
      setAllProducts(allProducts); // Armazena todos os produtos
      setProductsIgre(ingredientProducts);
    ...
    <Sidebar>
      ...
      <div className="container mx-auto mr-1.2 px-4 py-8 max-w-6xl">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-black">Lista de Produtos</h1>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 rounded-full text-xs ${product.isDerived ? 'bg-blue-500 text-white' : 'bg-gray-600 text-white'}`}>
                        {product.isIgredient ? 'Igrediente' : 'Produto'}
                      </span>
                    </td>
